...
namespace CharacterCreator2D
{
    ...
    public static class ResourcesManager
    {
        ...
        private class ResourcesUnloader : MonoBehaviour
        {
            private bool m_isUnloading;

            public void RequestUnloadUnused()
            {
                m_isUnloading = true;
            }
            ...
            private void Update()
            {
                ...
                {
                    m_isUnloading = false;
                    ReleaseZeroReference();
                }
            }
        }

        private static ResourcesUnloader m_resUnloader;
        ...
        public static void RequestReleaseZeroReference()
        {
#if !UNITY_EDITOR && CC2D_RES
            if (!m_resUnloader)
            {
                GameObject go = new GameObject(nameof(ResourcesUnloader));
                Object.DontDestroyOnLoad(go);
                go.hideFlags = HideFlags.HideAndDontSave;
                m_resUnloader = go.AddComponent<ResourcesUnloader>();
            }
            m_resUnloader.RequestUnloadUnused();
#endif
        }
    }
}
